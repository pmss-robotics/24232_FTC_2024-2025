...
public class OneDriverTeleOp extends CommandOpMode {
    ...
    public void initialize() {
        ...
        new GamepadButton(driver, GamepadKeys.Button.RIGHT_BUMPER).whenPressed(
                new SequentialCommandGroup(
                        ...
                        new InstantCommand(() -> arm.shoulderHome(0.3)),
                        new InstantCommand(() -> arm.shoulderMoveTo(arm.ps_Home), arm),
                        new WaitCommand(1000),
                        new InstantCommand(() -> arm.shoulderHome(1))
                )
        );
    ...
    /*public Command traj180command(Pose2d current) {
        Action traj180 = drive.actionBuilder(current)
                .turnTo(Math.toRadians(90))
                .build();
        return new TeleActionCommand(traj180, drive, Stream.of(drive).collect(Collectors.toSet()));
    }*/
}